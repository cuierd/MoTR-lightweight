...
class associationMerger:
    ...
    def write_out_denoise_merged_associations(self) -> None:
        ...
        trial_data_path = Path('../data/en/trials/filtered_preprocessed_onestop_en.csv')
...
if __name__ == '__main__':
    ...
    input_folder = Path('../data/en/divided')
    output_association_path = Path('../data/en/associations/')
    for file in input_folder.glob('*.csv'):
        print('I am identifying associations for :', file)
        obj_merger = associationMerger(file, output_association_path, 160, 4000)
